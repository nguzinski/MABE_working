...
json keyIssuing(json authStores,  pairing_t pairing, element_t g1){ 
    //USER SIGNUP
    json userInfo;
    std::cout << "Starting user signup process..." << std::endl;
    ...
    std::cout << "Generated user GID." << std::endl;

    json::iterator it_k = authStores.begin();
    while ( it_k !=  authStores.end() ) {
        json auth_k = it_k.value();
        std::cout << "Processing auth store with ID: " << auth_k["ID"] << std::endl;
        ...
        while ( it_j !=  authStores.end() ) {
            ...
            std::cout << "Creating R_ID: " << R_ID << std::endl;

            element_t R_u;
            element_init_Zr(R_u,pairing);
            element_random(R_u);            
            userInfo[R_ID] = convertToString(R_u);
            std::cout << "Generated R_u for " << R_ID << std::endl;
            ...
            if ( auth_k["ID"] > auth_j["ID"] ){
                ...
                std::cout << "Computed D_k_j for " << D_ID << std::endl;
            }
            else {
                ...
                //Perform PRF = y_k^(x_j * 1/(s_j-k + GID))
                element_init_Zr(temp_1, pairing);
                element_init_Zr(temp_2, pairing);
                element_add(temp_1,s,GID);
                element_div(temp_2,x,temp_1);
    ...
    std::cout << "Completed processing auth stores for user signup." << std::endl;

    //POLYNOMIAL CALCULATION setting d_k = 2
    it_k = authStores.begin();
    while ( it_k !=  authStores.end() ) {    
        json auth = it_k.value();
        std::cout << "Calculating coefficients for auth ID: " << auth["ID"] << std::endl;
        element_t R_All, p_, v, coeff[2];
        element_init_Zr(v, pairing); 
        element_init_Zr(p_, pairing);
        ...
        while ( it_j !=  authStores.end() ) {      
            ...
            int k_ID =  auth["ID"]; 
            int j_ID =  otherAuth["ID"];
            std::string R_ID =  "R_" + std::to_string(k_ID) + "_" +  std::to_string(j_ID);
            std::cout << "Adding R_k_j for " << R_ID << std::endl;
        ...
        std::cout << "Stored coefficients for auth ID: " << ID << std::endl;
        it_k++;   
    }

    //TODO: grab total attr from somewhere editable
    //have this grabbed from the public json or something, this prevents it from looping too many times
    int totalAttr = 4;
    json::iterator it = authStores.begin();
    while ( it !=  authStores.end() ) {
        json auth = it.value();
        int attr_num = auth["attr_num"]; // Get the number of attributes
        int found_attrs = 0; // Counter for found t_a attributes
        int i = 1; // Start from 1 to match the suffix of t_a
        
        while (found_attrs < attr_num && i <= totalAttr ) { // Continue until we find attr_num t_a's
            std::string atrr_ID = "t_a" + std::to_string(i);
            std::cout << "Current atrr_ID: " << atrr_ID << std::endl;
            if (auth.find(atrr_ID) != auth.end()) { // Check if t_a exists
                int authID = auth["ID"];
                ...
                std::cout << "Computed S_ID: " << S_ID << " for auth ID: " << authID << std::endl;

                found_attrs++; // Increment the count of found t_a attributes
            }
            i++; // Increment to check the next t_a
        }
        it++;
    }
    ...
    std::cout << "Finished key issuing process." << std::endl;
    return userInfo;
}
...
void decrypt( json enc_msg, json userInfo, pairing_t pairing, element_t g1, element_t g2, element_t e_g1g2, element_t msg, element_t decrypted_msg){
	...
	while ( it !=  enc_msg.end() ) {
        ...
        std::cout << "Currently processing attribute: " << attribute << " for authority: " << auth << std::endl;
        std::cout << "S_ID: " << S_ID << std::endl;
        
        if (userInfo.find(S_ID) == userInfo.end()) {
            std::cerr << "Error: User information for " << S_ID << " not found." << std::endl;
            std::cerr << "Decryption process cannot continue." << std::endl;
            return;
        }
    ...
    std::cout << "Decrypted Message: " << decrypted_msg - 1 << std::endl;
    ...
    json decryptData;
    decryptData["msg"] = convertToString(msg);
    decryptData["decrypted_msg"] = convertToString(decrypted_msg);

    std::ofstream outFile("decrypt.json");
    if (outFile.is_open()) {
        outFile << decryptData.dump(4); // Pretty print with 4 spaces
        outFile.close();
    } else {
        std::cerr << "Unable to open file decrypt.json for writing." << std::endl;
    }
    }
    else { 
